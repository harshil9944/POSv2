...
class Item_composites extends MY_Controller {

    public $module = 'item_composites';
    public $model = 'item_composite';
    public $singular = 'Composite Item';
    public $plural = 'Composite Items';
    public $edit_form = '';
    ...
    public function index()	{
        ...
        $page = [
            ...
            'plural'        =>  $this->plural,
            'add_url'       =>  base_url('items/item_composites/add'),
            'vue_add_url'   =>  '',
            'table'         =>  $table,
            'edit_form'     =>  ''
        ];
    ...
    public function add() {
        ...
        _set_layout('item_composites_form_view');
    ...
    public function _action_put() {
        ...
        $this->_prep_obj($obj);
        $meta = $obj['meta'];
        $item_composites = (isset($meta['item_composites']) && is_array($meta['item_composites']))?$meta['item_composites']:[];
        $obj['status'] = 1;
        $obj['added'] = sql_now_datetime();
        unset($obj['meta']);
        unset($obj['id']);
        unset($meta['item_composites']);

        if($this->{$this->model}->insert($obj)) {
            $id = $this->{$this->model}->insert_id();
            $this->{$this->model}->insert_meta($meta,$id);
            $this->{$this->model}->update_cluster_links($item_composites,$id);
    ...
    public function _action_post() {
        ...
        if($this->{$this->model}->update($obj,$filter)) {
            ...
            $this->{$this->model}->update_cluster_links($item_composites,$filter['id']);
            _response_data('redirect',base_url($this->module));
        }else{
            _response_data('message','Something wrong. Please try again.');
        }
    ...
    protected function _load_files() {
        ...
        if(_get_method()=='add' || _get_method()=='edit') {
            ...
            _page_script_override('item_composites/item_composites-form');
        }
