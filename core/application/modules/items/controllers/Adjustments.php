...
class Adjustments extends MY_Controller {
    ...
    public $model = 'adjustment';
    public $singular = 'Adjustment';
    public $plural = 'Adjustments';
    public $edit_form = '';
    ...
    public function index()	{
        ...
        $page = [
            ...
            'plural'        =>  $this->plural,
            'add_url'       =>  base_url('items/adjustments/add'),
            'vue_add_url'   =>  '',
            'table'         =>  $table,
            'edit_form'     =>  ''
        ];
    ...
    public function add() {
        ...
        _set_layout('adjustments_form_view');
    ...
    public function _action_put() {
        ...
        $this->_prep_obj($obj);
        $meta = $obj['meta'];
        $adjustments = (isset($meta['adjustments']) && is_array($meta['adjustments']))?$meta['adjustments']:[];
        $obj['status'] = 1;
        $obj['added'] = sql_now_datetime();
        unset($obj['meta']);
        unset($obj['id']);
        unset($meta['adjustments']);

        if($this->{$this->model}->insert($obj)) {
            $id = $this->{$this->model}->insert_id();
            $this->{$this->model}->insert_meta($meta,$id);
            $this->{$this->model}->update_cluster_links($adjustments,$id);
    ...
    public function _action_post() {
        ...
        if($this->{$this->model}->update($obj,$filter)) {
            ...
            $this->{$this->model}->update_cluster_links($adjustments,$filter['id']);
            _response_data('redirect',base_url($this->module));
        }else{
            _response_data('message','Something wrong. Please try again.');
        }
    ...
    protected function _load_files() {
        ...
        if(_get_method()=='add' || _get_method()=='edit') {
            ...
            _page_script_override('adjustments/adjustments-form');
        }
