...
Vue.mixin({
    methods: {
        ...
        generateOrderItemId: function(itemId, skuId) {
            return itemId + "-" + skuId + "-" + Math.floor(Math.random() * 100) + 1;
        },
...
var promotionMixin = {
    ...
    methods: {
        isCartItem: function(itemId, skuId) {
            var item = this.order.cart.items.findIndex(function(i) {
                return i.itemId === itemId && i.skuId === skuId;
            });
            return item !== -1;
        },
        ...
        getItemName: function(itemId, skuId) {
            var item = this.getItem(itemId);
            if (item.type === "single") {
                return item.name;
            } else {
                var sku = item.variations.find(function(v) {
                    return v.skuId === skuId;
                });
                return item.name + " (" + sku.title + ")";
            }
        },
        getItemPriceText: function(itemId, skuId) {
            var price = 0;
            var item = this.getItem(itemId, skuId);
            if (item.type === "group") {
                var variation = item.variations.find(function(ip) {
                    return ip.itemId === itemId && ip.skuId === skuId;
                });
                price =
                    typeof variation.salePrice !== "undefined" ? variation.salePrice : 0;
            } else {
                price = item.prices[0].salePrice;
            }
            return this.beautifyCurrency(this.toTwoDecimal(price));
        },
        getItemDiscountedPriceText: function(itemId, skuId) {
            var price = 0;
            var item = this.getItem(itemId, skuId);
            if (item.type === "group") {
                var variation = item.variations.find(function(ip) {
        ...
        getAvailablePromotions: function(itemId, skuId) {
            var promotions = [];
                    ...
                    if (criteria) {
                        ...
                        if (criteriaIncludes) {
                            criteriaIncludes.forEach(function(cInc) {
                                if (
                                    cInc.itemId.toString() === itemId.toString() &&
                                    cInc.skuId.toString() === skuId.toString()
                                ) {
                                    addPromotion = true;
                                }
                            });
                    ...
                    if (reward) {
                        ...
                        if (rewardIncludes) {
                            rewardIncludes.forEach(function(rInc) {
                                if (
                                    rInc.itemId.toString() === itemId.toString() &&
                                    rInc.skuId.toString() === skuId.toString()
                                ) {
                                    addPromotion = true;
                                }
                            });
                    ...
                    basicPromotions.forEach(function(bp) {
                        ...
                        self.cart.items.forEach(function(item, index) {
                            if (productType === "all") {
                                ...
                                if (
                                    self.hasPromoItem(item.itemId, item.skuId, reward.include)
                                ) {
                                    addBp = true;
                                    total += Number(
                                        self.getDiscountAmount(
                        ...
                        cartItems.forEach(function(item) {
                            if (criteria) {
                                if (criteriaProductType === "include") {
                                    if (
                                        self.hasPromoItem(item.itemId, item.skuId, criteria.include)
                                    ) {
        ...
        hasPromoItem: function(itemId, skuId, obj) {
            var index = obj.findIndex(function(s) {
                return s.itemId === itemId && s.skuId === skuId;
            });
            return index !== -1;
        },
        ...
        getApplicablePromotions: function() {
            ...
            this.cart.items.forEach(function(i) {
                var itemId = i.itemId;
                var skuId = i.skuId;
                var itemAvailablePromotions = self.getAvailablePromotions(
                    itemId,
                    skuId,
                );
    ...
    methods: {
        ...
        handleAddToCart: function(item) {
            ...
            if (existingItemIndex === -1) {
                var cartItem = {
                    id: null,
                    orderItemId: this.generateOrderItemId(item.id, item.skuId),
                    type: item.type,
                    itemId: item.id,
                    skuId: item.skuId,
                    baseUnitId: item.unit,
                    saleUnitId: item.saleUnit,
                    sku: item.sku,
                ...
                if (item.type !== "group" && !item.rate) {
                    var priceGroup = item.prices.find(function(price) {
                        return price.itemId === item.id && price.skuId === item.skuId;
                    });
                    cartItem.rate = cartItem.unitRate = priceGroup.salePrice;
                } else {
        ...
        getExistingItem: function(itemId, skuId) {
            return this.cart.items.find(function(item) {
                return item.itemId === itemId && item.skuId === skuId;
            });
        },
...
Vue.component("cart-edit-item", {
        ...
        getGroupedAddons: function() {
            ...
            this.obj.addons.forEach(function(a) {
                var index = addons.findIndex(function(fa) {
                    return a.addonItemId === fa.addonItemId;
                });
                if (index === -1) {
                    addons.push(a);
                }
    ...
    methods: {
        calculatePrice: function() {
            ...
            addonPrice = selectedAddons.reduce(function(total, selectedAddon) {
                if (selectedAddon) {
                    return (
                        Number(total) +
                        Number(selectedAddon.salePrice) * Number(selectedAddon.quantity)
                    );
                }
                return Number(total);
            }, addonPrice);
        ...
        updateAddonSelection: function(e) {
            var addonItemId = e.target.getAttribute("data-addon-item-id");
            var self = this;
            this.obj.addons.forEach(function(ia, index) {
                if (addonItemId === ia.addonItemId) {
            ...
            selectedAddons.forEach(function(sa) {
                self.obj.addons.forEach(function(a) {
                    if (a.addonItemId === sa.addonItemId) {
                        a.enabled = true;
                    }
                });
            });
        ...
        handleItemClick: function(index) {
            if (this.isEditable) {
                ...
                var existingItemIndex = this.cart.items.findIndex(function(single) {
                    return single.itemId === item.id && single.type !== "group";
                });
                ...
                if (existingItemIndex === -1) {
                    ...
                    var existingItem = this.cart.items[existingItemIndex];
                    if (!existingItem.data.addons.length &&
                        !existingItem.data.notes.length
                    ) {
                        if (existingItem.type === "single") {
...
Vue.component("group-item-detail", {
    ...
    watch: {
        ...
        customPrice: {
            handler: function(after, before) {
                this.item.variations[0].salePrice = after;
            },
        },
    },
    computed: {
    ...
    methods: {
        calculatePrice: function() {
            ...
            this.item.variations.forEach(function(variation) {
                ...
                addonPrice = selectedAddons.reduce(function(total, selectedAddon) {
                    if (
                        selectedAddon.itemId === variation.itemId &&
                        selectedAddon.skuId === variation.skuId
                    ) {
                        return (
                            Number(total) +
                            Number(selectedAddon.salePrice) * Number(selectedAddon.quantity)
                ...
                var generalAddonPrice = selectedAddons.reduce(function(
                        ...
                        if (Number(selectedAddon.skuId) === 0) {
                            return (
                                Number(total) +
                                Number(selectedAddon.salePrice) * Number(selectedAddon.quantity)
                            );
        ...
        handleGroupItemDetailInit: function() {
            if (this.item.type === "group") {
                ...
                if (typeof this.item.baseName === "undefined") {
                    this.item.baseName = this.item.data.baseName;
                }
                this.item.variations.forEach(function(single) {
                    single.originalPrice = single.salePrice;
                });
            } else {
                ...
                var variation = {
                    ...
                    itemId: this.item.id,
                    originalPrice: this.item.prices[0].salePrice,
                    salePrice: this.item.prices[0].salePrice,
                    sku: this.item.sku,
                ...
                this.item.baseName = this.item.name;
        ...
        handleAddToCart: function(index) {
            ...
            this.item.id = variation.itemId;
            this.item.skuId = variation.skuId;
            this.item.rate = variation.salePrice;

            this.item.addons.forEach(function(ai) {
                if (Number(ai.skuId) !== 0) {
                    if (ai.skuId !== variation.skuId) {
                        ai.enabled = false;
                    }
                }
            });
            ...
            if (this.item.type === "group") {
                this.item.name = this.item.baseName + " - " + variation.title;
            } else {
                this.item.name = this.item.baseName;
            }
    ...
    methods: {
        ...
        populateMeta: function() {
            ...
            request.then(function(response) {
                ...
                var partialRefund = self.getFilteredOrders("Partial Refunded");
...
Vue.component("session-summary", {
    ...
    created: function() {
        var self = this;
        bus.$on("showSessionSummary", function() {
...
Vue.component("split-order", {
    ...
    computed: {
        ...
        remainingItem: function() {
            return this.hasNonAddedItems ? this.activeInvoice + 1 !== this.order.split.length : true;
        }
    },
    methods: {
        initSplit: function() {
        ...
        getSplitInvoicesItemTotalQty: function(item) {
            ...
            if (this.order.split.length) {
                this.order.split.forEach(function(s) {
                    ...
                    //console.log('Single Split Item Qty: ' + itemTotalQty);
        ...
        getItemTotalQty: function(items, compareItem) {
            ...
            console.log('Total Qty ' + itemId + '/' + skuId + ': ' + totalQty);*/
        ...
        getNonAddedCartItems: function() {
            ...
            cartItems.forEach(function(ci) {
                ...
                //console.log(ci.title + ' ' + totalQuantityAdded);
            ...
            await asyncForEach(self.nonAddedCartItems, async(i) => {
                if (Number(i.quantity) > 0) {
                    var cartItem = self.order.cart.items.find(function(ci) {
                        return ci.itemId === i.itemId && ci.skuId === i.skuId;
                    });
        ...
        handleAddSplitItem: function(item) {
            ...
            if (itemIndex === -1) {
                var cartItem = this.order.cart.items.find(function(ci) {
                    return ci.itemId === itemId && ci.skuId === skuId;
                });
                ...
                var totalQuantityAdded = this.getSplitInvoicesItemTotalQty(
                    itemId,
                    skuId,
                );
                var existingQty = this.getItemTotalQty(
        ...
        calculateItem: function() {
            ...
            self.order.split.forEach(function(invoice) {
                ...
                mixTotal -= Number(promotionTotal);
                var discount = Number(subTotal) * Number(self.order.cart.totals.discount) / Number(self.order.cart.totals.subTotal);
...
Vue.component("pos", {
    ...
    methods: {
        ...
        handleSessionSummary: function() {
            bus.$emit("showSessionSummary", true);
        },
        ...
        resetOrder: function(payload) {
            ...
            if (resetOrder) {
                this.order = {
                    ...
                    warehouseId: 1,
                    registerId: this.session.registerId,
        ...
        updateCheck: function() {
            ...
            if (self.canCheckUpdate) {
                ...
                var request = submitRequest(data, "get");
                request.then(function(response) {
                    /*if(response.appVersion !== _s('appVersion')) {
                        window.location.reload();
                        return false;
                    }*/
                    if (response.printQueueCount >= _s("printQueueWarningLimit")) {
                        self.showTopWarning(
                    ...
                    if (response.printQueue.length) {
                        bus.$emit("initQueuePrint", response.printQueue);
                    }

                    bus.$emit("itemCountUpdate", response.itemCount);
                    if (response.onlineOrderCount) {
        ...
        getOrderItemId: function() {
            ...
            if (items) {
                items.forEach(function(item) {
                    item.orderItemId = self.generateOrderItemId(item.itemId, item.skuId);
                });
            }
            return true;
        },
